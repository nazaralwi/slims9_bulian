...
if (isset($_POST['saveData']) AND $can_read AND $can_write) {
    ...
    if (empty($memberID) OR empty($memberName) OR empty($birthDate)) {
         ...
         */
	// Hook custom fields
     	\SLiMS\Plugins::getInstance()->execute('member_custom_fields', ['member_custom_fields' => &$member_custom_fields]);
...
<div class="menuBoxInner memberIcon">
    ...
    <form name="search" action="<?php echo MWB; ?>membership/index.php" id="search" method="get" class="form-inline"><?php echo __('Search'); ?>
      ...
      $customfield = $dbs->query('select dbfield,label,data,type from mst_custom_field where primary_table = \'member\'');

      $cfData = [];
      if ($customfield->num_rows > 0)
      {
        echo '<select name="customfield" class="form-control col-md-2">';
        echo '<option value="0">Pilih</option>';
        while ($data = $customfield->fetch_assoc()) {
          if (in_array($data['type'], ['dropdown','checklist','choice'])) 
            $cfData[$data['dbfield']] = $data['data'];

          echo '<option value="'.$data['dbfield'].'">' . $data['label'] . '</option>';
        }
        echo '</select>';
      }
      // End modified
...
<?php
    ...
    if ($customfield->num_rows > 0)
    {
      $table_spec .= ' LEFT JOIN member_custom AS mc ON mc.member_id = m.member_id ';
    }
    ...
    if (isset($_GET['keywords']) AND $_GET['keywords']) {
      ...
      //  Modified by Drajat Hasan
       if ($customfield->num_rows > 0 && isset($_GET['customfield']))
       {
         $cf = $dbs->escape_string($_GET['customfield']);

         if (isset($cfData[$cf]))
         {
           foreach (unserialize($cfData[$cf]) as $id => $data) {
              if (preg_match('/'.$keywords.'/i', $data[1]))
              {
                $keywords = $data[0];
              }
           }
         ...
         $criteria = "m.member_id IS NOT NULL AND {$cf} LIKE '%" . $dbs->escape_string($keywords) . "%' ";
       }
      //  end modified
    }
    if (isset($_GET['expire'])) {
